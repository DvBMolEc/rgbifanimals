...
# requires installation of package "rstudioapi"
...
# Find the shortest path for each sampling location to the species observation
sapply(df$Specieslist, function(species) {
  print(species)
  tryCatch(find_closest_registered_place(species, coordinates, tr,
                                         "Output/ShortestPath.csv"),
...
apply(long, 1, function(row) {
  samplelocation <- coordinates[coordinates$Observatory.ID == row[2],
                                c("Longitude", "Latitude")]
  print(row[1])
  tryCatch(
           {occurrence_data <- check_occurrence_data(row[1])
            find_shortest_route_in_sea(samplelocation, occurrence_data, tr, row,
                                       "Output/DistanceOverSea.csv")},
           error = function(errormessage) {
             write.table(paste(c(row, 0, 0, NA), collapse = ","),
                         file = "Output/DistanceOverSea.csv", append = TRUE,
...
write.csv(results, "Output/DistanceOverSea.csv", quote = FALSE,
          row.names = FALSE)
